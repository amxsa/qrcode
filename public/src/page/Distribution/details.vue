    ...
    <div class="tab-content">
        <ul class="block">
            <li style="float: right;margin-right:100px;">
                <el-button :plain="true" type="info" @click="AddPersonnel"> + 添加分成方</el-button>
                <el-button :plain="true" type="info" @click='blackPage' style="display:block;float:right;margin-left:20px;">返回</el-button>
            </li>
        </ul>
        <div class="box_table">
        ...
        <!--添加分成方-->
        <el-dialog title="添加分成方" v-model="personnelS" :close-on-click-modal="false">
            <el-form :model="ruleForm" label-width="80px" :rules="ruleFormRules" ref="ruleForm">
                ...
                <el-form-item label="分成方名称:" v-if="ruleForm.split_type==2" label-width="120px">
                    <el-select v-model="split_target_Uuid" filterable="" remote placeholder="请输入组织架构获取岗位" :remote-method="OrgMethod" :loading="loading" style="width:100%" @change="orgChang">
                        <el-option v-for="item in OrgPost" :key="item.uuid" :label="item.name" :value="item.uuid">
                        </el-option>
                    </el-select>
                    <el-radio-group v-if="jobSwitch" v-model="ruleForm.split_target" @change="JOBId">
                        <el-radio v-for="item in org_JobArray" :label="item.aj_uuid" style="padding:10px 10px;">{{item.job_type}}({{item.realname}})</el-radio>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="分成方名称:" v-if="ruleForm.split_type==3" label-width="120px">
                    <el-select v-model="split_target_Uuid" filterable="" remote placeholder="请输入组织架构名称" :remote-method="remoteMethod" :loading="loading" style="width:100%" @change="orgTwo_Chang">
                        <el-option v-for="item in organizational" :key="item.uuid" :label="item.name" :value="item.uuid">
                        </el-option>
                    </el-select>
                </el-form-item>
        ...
        <!--编辑分成方-->
        <el-dialog title="编辑分成方" v-model="EditAccounts" :close-on-click-modal="false">
